...
function Show(Me,create){
	switch(Me[0]){
		case "Folder":
			draw_sprite_stretched(Spr_Menu,0,x,x,room_width-64*2,room_height-64*2)
			if(create == 0){
				
				for(var i = 0; i < array_length(Me[1]); i++){
					if(Me[1][i][3] != 0){
						var App = instance_create_depth(x+128+96*i,y+128,-room_height-1,Obj_App)
						App.image_index = Me[1][i][0]
						App.func = Me[1][i][1]
						App.nome = Me[1][i][2]
						
					}
				}
				var Close = instance_create_depth(x+room_width-64*2-96,y+96,-room_height-1,Obj_App)
				Close.image_xscale = 0.7
				Close.image_yscale = 0.7
				Close.image_index = 3
				Close.func = function(){Me[2](); global.Menu = -1;}
